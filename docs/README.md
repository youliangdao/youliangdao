...
株式会社 COUNTERWORKS にて、フロントエンドエンジニアとして Next.js と React を用いた SaaS の開発に約 1 年半従事しています。基本設計や詳細設計から開発、テスト、リリース、運用保守まで一貫して担当しています。また開発業務の他にスクラムマスターとして 4 名のエンジニアをマネジメントしつつ進捗管理やタスク切り出しもしております。直近では副業で約半年ほど自社サービスの開発に参画しており、フロントエンドだけでなく DB 設計や AWS Lambda・SES を用いたインフラ構築なども行っておりました。
...
B to B のパートナーセールス業界に特化した Vertical SaaS を開発。

副業として参画し、活動ログ機能の画面作成と DB 設計や AWS Lambda と SES を用いたメール配信機能等を担当。

- 活動ログ機能のフロントエンド実装と DB 設計を担当
- ユーザー体験を損なわたいために GrahpQL による無限スクロール機能の実装
- パフォーマンス面を考慮して仮想スクロールを導入
- Slack や LINE のような慣れ親しんだ UI 設計にするために逆順スクロールを実装
- 収集した活動ログを保存するための DB 設計も行った
- 特定ドメイン宛に送信されたメールを収集するシステムの構築
- AWS SES・Lambda を用いたインフラ基盤の構築を主に担当
- Lambda 関数をコンテナイメージとともにデプロイし、Lambda 上で ORM を操作し、バックエンド側の DB に保存できるよう実装
...
得意としているフロントエンド開発の知見を活かして、フロントエンド基盤の改善や既存機能の改修をリード。

開発初期は人の入れ替わりが激しく負債化しているコードが多数存在していたため、コンポーネント設計の指針を定めて、誰でも統一されたコードを書けるようにしました。さらに並行してテスト文化も根付いていなかったため、組織内で議論を重ね自らが旗振り役になり、Storybook などを導入してテストを積極的に書くよう工夫しました。

また機能開発というスコープに閉じず、チーム間での横断した開発にも精力的に取り組んでおりました。具体的には次のような取り組みを行っておりました。
...
- 開発環境の整備
- ディレクトリ構造の整備
- ESLint / Prettier を用いてコーディング規約を整備
- ADR などを用いたドキュメント文化の定着

さらに事業部制に移行してからは、スクラムマスターとして 4 名のエンジニアをマネジメントしつつ各種スクラムイベントの運営をしておりました。

進捗管理や PO との仕様のすり合わせ、タスクの切り出し等を行いつつ日々のデイリースクラムやリファインメントでチームが円滑に開発を進められるよう工夫しました。
...
- プロジェクト詳細：
  - Next.js を利用して SSG / SSR / CSR を使い分け画面を実装
  - タスク管理機能の実装
  - 商談管理機能の実装
  - 各区画の稼働状況や空き状況を管理しやすくするためのカレンダー UI をリニューアル
    - ライブラリを用いずにカレンダー機能を実装しました
    - 大量の施設データを扱う必要性があったため、パフォーマンス面を考慮し開発を行いました
  - 収入管理機能の実装
  - ダッシュボード機能の実装
  - Tanstack Table を用いたビューテーブル機能の実装
    - 一覧ページをビューテーブルへ移行し、閲覧者が見たい形でデータを整理できるように工夫
    - 約 3000 件のデータをフロントエンド側で扱えるようパフォーマンス観点の側面から仮想スクロールを導入
    - バックエンドの API のタイムアウトエラーに備えて、API のリトライ機能を実装
    - 画面離脱時にアラートを出すために、離脱アラートを実装
  - Tanstack Query（旧 React Query）を用いた API 通信の実装
  ...
  - React Hook Form を用いた複雑なフォーム管理の実装
    ...
    - EOF 最適化
  - Suspense・ErrorBoundary を用いた権限機能の実装
    - フロントエンド側にて権限周りの開発をよりスムーズに整備
    - 同時に Tanstack Query を v5 へ移行し、Suspense での開発をよりシームレスに行えるように工夫
  ...
  - Storybook の導入と単体〜結合テストの実装
  - Playwright を用いた E2E テストの実装
- その他：
  - AWS・Terraform・CloudFlare などを用いた導入企業のインフラ構築と外形監視ツール（New Relic・Sentry）の導入
  ...
  - フロントエンドのビルド時間の短縮
    ...
    - Knip を用いて不要なコードと不要パッケージを削除
    - パッケージ管理を yarn → pnpm へ移行
  - Plop を用いた雛形コンポーネント作成フローを整備
  - Next.js を v12.3 → v13.5 へ移行
  - Docker 環境を Volta と pnpm を用いたローカル環境開発へ移行し、開発生産性を向上
  - コンポーネントのディレクトリ構造を整備し、依存関係やコンポーネントの責務をより明確にした
  - ESLint / Prettier にいくつか独自ルールを追加し、フロントエンドの開発基盤を整備
  - テックブログ運営
  - ADR、コーディング規約を積極的にドキュメントに残す。開発者が開発しやすい環境を作る
